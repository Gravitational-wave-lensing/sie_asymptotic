...
def fisher_information_matrix( log_T_star, log_dL, f, source_r, source_phi, log_dL_effectives_median, log_time_delays_median, log_sigma_dL_effectives=jnp.ones(4)*0.1, log_sigma_time_delays=jnp.ones(3)*0.03, omegatilde=0):
    ...
    chi_squared_grad = grad(chi_squared)
    chi_squared_hessian = hessian(chi_squared, argnums=(0, 1, 2, 3, 4))
    fisher_information = chi_squared_hessian(log_T_star, log_dL, f, source_r, source_phi, log_dL_effectives_median, log_time_delays_median, log_sigma_dL_effectives, log_sigma_time_delays, omegatilde)/2
...
if __name__ == "__main__":
    ...
    log_time_delays = log_T_star + jnp.log(fermat_polar - fermat_polar[0])
